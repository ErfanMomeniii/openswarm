...
- Starter templates now give every agent `max_tokens: 4096` (was 2048 for workers). Reasoning models spend tokens thinking before they answer, and the lower budget truncated them mid-thought. The README documents how to size this.

### Fixed
- **`openswarm-mcp` failed to start on new installs.** mcp 2.0 renamed `FastMCP` to `MCPServer` and dropped the `mcp.server.fastmcp` module, so every fresh `pip install "openswarm-ai[mcp]"` produced an IDE integration that could not launch. The server now resolves whichever class the installed mcp provides; both 1.x and 2.x are supported.
- **Generated code came back as a single unusable line.** When a model's JSON was slightly malformed — a stray unescaped quote is enough — the parser fell back to a regex path that returned the raw slice without unescaping, so `\n` stayed literal and a whole function arrived on one line. Escape sequences are now decoded on every parse path.
- **Pipeline stages were given the hierarchical protocol**, which offers `delegate`, `review`, and `question` — actions that cannot work in a pipeline. Models acted on them and returned reasoning about delegating instead of doing the work. Pipeline agents now get a protocol describing only what they can actually do.
- **Truncated responses were returned as if they were complete.** A reply cut off by `max_tokens` was passed on silently; with reasoning models, which spend the budget thinking before answering, that surfaced as the model's scratch work in place of a result. Truncation is now logged with the model name and the fix.
